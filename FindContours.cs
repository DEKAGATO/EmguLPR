...
namespace Auto_parking
{
    class FindContours
    {
        ...
        /// <param name="processedColor">Resulting color image.</param> will show image and seprate each char
        public int IdentifyContours(Bitmap colorImage, int thresholdValue, bool invert, out Bitmap processedGray, out Bitmap processedColor, out List<Rectangle> list_)
        {
                ...
                for (int s = -1; s <= 1 && s + value != 1; s += 2)
                {
                    ...
                    using (MemStorage storage = new MemStorage())
                    {
                        ...
                        while (contours != null)
                        {
                            ...
                            Rectangle rect = contours.BoundingRectangle;
                            CvInvoke.cvDrawContours(color2, contours, new MCvScalar(255, 0, 255), new MCvScalar(0), -1, 1, Emgu.CV.CvEnum.LINE_TYPE.EIGHT_CONNECTED, new Point(0, 0));
